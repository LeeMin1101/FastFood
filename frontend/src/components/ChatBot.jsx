  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 font-sans">
      <style>{`
        ...
        .animate-chat-open {
          animation: chatPopup 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
        }
        .animate-icon-click {
          animation: iconPulse 0.4s ease-out forwards;
        }
      `}</style>
          ...
          <div className="bg-gradient-to-r from-orange-500 to-red-500 p-4 text-white flex justify-between items-center shadow-lg z-10 rounded-t-2xl">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-white/80 font-medium">Sẵn sàng hỗ trợ</p>
              </div>
            </div>
          ...
          <div className="flex-1 p-4 overflow-y-auto bg-white/5 flex flex-col gap-4">
            ...
            {messages.length === 0 && (
              <div className="text-center mt-4 mb-2">
                ...
                <div className="flex gap-3 mt-6">
                  ...
                  <div className="bg-white/10 border border-white/20 text-gray-200 text-sm p-3 rounded-2xl rounded-tl-none shadow-lg">
            ...
            {messages.map((msg, idx) => (
              <div key={idx} className={`flex gap-2 ${msg.sender === "user" ? "justify-end" : "justify-start"}`}>
                {msg.sender !== "user" && (
                  <div className="w-8 h-8 bg-gradient-to-tr from-orange-400 to-red-500 rounded-full flex items-center justify-center text-white text-xs shrink-0 shadow-md">M</div>
                )}
                <div className={`p-3 max-w-[75%] text-sm shadow-lg ${
                  msg.sender === "user" 
                  ? "bg-gradient-to-r from-orange-500 to-red-500 text-white rounded-2xl rounded-tr-none" 
                  : "bg-white/10 border border-white/20 text-gray-200 rounded-2xl rounded-tl-none"
                }`}>
                  {msg.text}
                </div>
              </div>
